...
void degree_comp_full(double * degree_vec, int ** incidence_list, int * incidence_list_size, int R){
    int i, j;
    for(i = 0; i < R; i++){
        for(j = 0; j < incidence_list_size[i]; j++){
            degree_vec[incidence_list[i][j]]++;
        }
    }    
}
...
void main_func(double * y_sum, double * record, double * record_discrete,
        ...
        double ** parameter_list, int * parameter_list_size,
        char ** submodular_type, int * submodular_type_size,
        double * bias_vec, int N, int R, unsigned int T,
        unsigned int record_dis){
    double ** y = new double * [R];
    double * y_ave = new double [N], * temp_y_ave = new double [N];
    double a[maxe];
    double W[maxe];
    double newy[maxe];
    double * degree_vec = new double [N]();
    ...
    degree_comp_full(degree_vec, incidence_list, incidence_list_size, R);
    for(i = 0; i < R; i++){
        y[i] = new double [incidence_list_size[i]]();
    }
    for(i = 0; i < N; i++){
        y_ave[i] = bias_vec[i]/degree_vec[i];
    }

    for(t = 0; t < T; t++){
        for(i = 0; i < N; i++){
            temp_y_ave[i] = y_ave[i];
        }
        for(j = 0; j < R; j++){
            for(i = 0; i < incidence_list_size[j]; i++){
                a[i] = y[j][i] -temp_y_ave[incidence_list[j][i]];
                W[i] = degree_vec[incidence_list[j][i]];
            }
            if(submodular_type[j][0] == 'h'){
                projection_homo_weight(newy, a, W, parameter_list[j], incidence_list_size[j]);
            }
            if(submodular_type[j][0] == 'e'){
                projection_edge_weight(newy, a, W, parameter_list[j]);
            }
            if(submodular_type[j][0] == 'p'){
                projection_para_edge_weight(newy, a, W, parameter_list[j], incidence_list_size[j]);
            }
            if(submodular_type[j][0] == 'c'){
                projection_concave_card_weight(newy, a, W, parameter_list[j], incidence_list_size[j]);
            }
            for(i = 0; i < incidence_list_size[j]; i++){
                y_ave[incidence_list[j][i]] += (newy[i] - y[j][i])/W[i];
                y[j][i] = newy[i];
            }
        }
        if((t+1)%record_dis == 0){
            for(i = 0; i < N; i++){
                x[i] = -degree_vec[i]* y_ave[i];
                y_sum[i] = - x[i] - bias_vec[i];
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    ...
    unsigned int T = *(mxGetPr(prhs[6]));
    unsigned int record_dis = *(mxGetPr(prhs[7]));
