...
double gap_d_eff(int ** incidence_list, int * incidence_list_size,
    ...
    for(i = 1; i < N; i++){
        ...
        if(level_cost[i]< level_cost_min) level_cost_min = level_cost[i];
    ...
    delete[] level_cost;
    //mexPrintf("%f\n", pos_x_sum);
    return level_cost_min + pos_x_sum;
}
...
void main_func(double * y_sum, double * record, double * record_discrete,
        ...
        unsigned int record_dis, double c){
    double ** z = new double * [R];
    double ** u = new double * [R];
    double * z_sum = new double [N]();
    double * u_sum = new double [N]();
    int period = int(c*R/K);
    double theta = (K*1.0)/R, nexttheta , thetasq, deltaz, deltau;
    double ** newz = new double * [K];
    ...
    double para = 1, gap;
    for(i = 0; i < R; i++){
        z[i] = new double [incidence_list_size[i]]();
        u[i] = new double [incidence_list_size[i]]();
    }
    for(i = 0; i < K; i++){
        newz[i] = new double [maxe]; 
        a[i] = new double [maxe]; 
    ...
    for(t = 0; t < T; t++){
        if(t% period == 0){
            ...
            for(i = 0; i < R; i++){
                for(j = 0; j < incidence_list_size[i]; j++){
                    z[i][j] += thetasq*u[i][j];
                    u[i][j] = 0;
                    z_sum[incidence_list[i][j]] += z[i][j];
                }
            }
            nexttheta = (K*1.0)/R;
        }
        ...
        thetasq = theta * theta;
        for(i = 0; i < K; i++){
            picked[i] = distribution(generator);
        }
        degree_comp(degree_vec, incidence_list, incidence_list_size, picked, K);
        for(j = 0; j < K; j++){
            for(i = 0; i < incidence_list_size[picked[j]]; i++){
                a[j][i] = z[picked[j]][i] -
                        (K/theta)/R*(z_sum[incidence_list[picked[j]][i]] +
                        thetasq*u_sum[incidence_list[picked[j]][i]]+
                        bias_vec[incidence_list[picked[j]][i]])/degree_vec[j][i];
                //mexPrintf("%f\n", a[j][i]);
                //mexPrintf("a:%f, %d,%d\n", picked[j], i, a[picked[j]][i]);
                
            }
            if(submodular_type[picked[j]][0] == 'h'){
                projection_homo_weight(newz[j], a[j], degree_vec[j], parameter_list[picked[j]], incidence_list_size[picked[j]]);
            }
            if(submodular_type[picked[j]][0] == 'e'){
                projection_edge_weight(newz[j], a[j], degree_vec[j], parameter_list[picked[j]]);
            }
            if(submodular_type[picked[j]][0] == 'p'){
                projection_para_edge_weight(newz[j], a[j], degree_vec[j], parameter_list[picked[j]], incidence_list_size[picked[j]]);
            }
            if(submodular_type[picked[j]][0] == 'c'){
                projection_concave_card_weight(newz[j], a[j], degree_vec[j], parameter_list[picked[j]], incidence_list_size[picked[j]]);
            }
        }
        for(j = 0; j < K; j++){
            for(i = 0; i < incidence_list_size[picked[j]]; i++){
                deltaz = newz[j][i] - z[picked[j]][i];
                deltau = ((theta*R)/K - 1)/thetasq*deltaz;
                z_sum[incidence_list[picked[j]][i]] += deltaz;
                u_sum[incidence_list[picked[j]][i]] += deltau;
                z[picked[j]][i] = newz[j][i];
                u[picked[j]][i] += deltau;
            }
        }
        nexttheta = (sqrt(thetasq*thetasq + 4*thetasq) - thetasq)/2;
        if((t+1)%record_dis == 0){
            for(i = 0; i < N; i++){
                y_sum[i] = z_sum[i] + thetasq * u_sum[i];
            ...
            gap = gap_eff(incidence_list, incidence_list_size,
                    ...
                    N, R);
            mexPrintf("%d, gap: %f\n", t, gap);
            record[(t+1)/record_dis] = gap;
            gap = gap_d_eff(incidence_list, incidence_list_size,
                parameter_list, parameter_list_size,
                submodular_type, submodular_type_size,
                bias_vec, x, N, R);
            record_discrete[(t+1)/record_dis] = gap;
        }
    }
    delete[] z;
    delete[] u;
