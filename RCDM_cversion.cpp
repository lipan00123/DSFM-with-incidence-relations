    ...
    for(t = 0; t < T; t++){
        ...
        for(j = 0; j < K; j++){
            for(i = 0; i < incidence_list_size[picked[j]]; i++){
                a[j][i] = y[picked[j]][i] -
                        (y_sum[incidence_list[picked[j]][i]]+ bias_vec[incidence_list[picked[j]][i]])/degree_vec[j][i];
                //mexPrintf("%f\n", a[j][i]);
                //mexPrintf("a:%f, %d,%d\n", picked[j], i, a[picked[j]][i]);
                
            }
            if(submodular_type[picked[j]][0] == 'h'){
                projection_homo_weight(newy[j], a[j], degree_vec[j], parameter_list[picked[j]], incidence_list_size[picked[j]]);
            }
        ...
        for(j = 0; j < K; j++){
            for(i = 0; i < incidence_list_size[picked[j]]; i++){
                y_sum[incidence_list[picked[j]][i]] += newy[j][i] - y[picked[j]][i];
                y[picked[j]][i] = newy[j][i];
            }
        }
        if((t+1)%record_dis == 0){
            for(i = 0; i < N; i++){
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
    // [y, record, record_discrete] = RCDM_cversion(incidence_list, parameter_list, submodular_type, bias_vec, N, R, K, T, record_dis);
	if (nlhs != 3 || nrhs != 9) {
